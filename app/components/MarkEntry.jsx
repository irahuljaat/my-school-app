            ...
            <div className="max-w-[1600px] mx-auto space-y-4">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-4 flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        {/* RESTORED: ATTENDANCE BUTTON */}
                        <button 
                            onClick={() => router.push('/exam-manage/attendance')} 
                            className="flex items-center gap-2 px-4 py-2.5 bg-amber-50 text-amber-700 rounded-xl font-bold text-[11px] uppercase tracking-wider hover:bg-amber-100 transition-all border border-amber-200/50"
                        >
                        ...
                        <div className="flex items-center gap-1.5 ml-1">
                            <button onClick={() => fileInputRef.current.click()} className="p-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition-all" title="Upload Marks">
                                <HiOutlineUpload size={18} />
                            </button>
                            <input type="file" ref={fileInputRef} hidden accept=".xlsx, .xls" onChange={handleFileUpload} />

                            <button onClick={downloadTemplate} className="p-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition-all" title="Download Excel Template">
                        ...
                        <button 
                            onClick={handleSave} 
                            disabled={isSaving}
                            className="flex items-center gap-2 px-6 py-2.5 bg-slate-900 text-white rounded-xl font-bold text-[11px] uppercase tracking-wider hover:bg-indigo-600 disabled:opacity-50 transition-all shadow-lg shadow-slate-200 ml-2"
                        >
                                                    ...
                                                    <td key={sub.name} className="p-2 text-center">
                                                        {isSubjectApplicable(stu, sub.name) ? (
                                                            <input
                                                                ...
                                                                className="w-16 py-2.5 text-center text-xs font-black bg-slate-50 border border-transparent rounded-xl focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all placeholder:text-slate-200"
                                                            />
                                                        ) : (
                                                            <div className="h-1 w-6 bg-slate-100 mx-auto rounded-full" title="Subject Not Applicable" />
                                                        )}
                                                    </td>
                                                ))}
                                            </tr>
